...
function MessStaffTools() {
  ...
  const addMenuItem = (e) => {
    ...
    if (newMenuItem.name && newMenuItem.price) {
      setMenuItems([...menuItems, { id: menuItems.length + 1, name: newMenuItem.name, price: parseFloat(newMenuItem.price) }]);
          ...
          <tbody>
            {stockItems.map((item) => (
              <tr key={item.id}>
                ...
                <td>{item.unit}</td>
                <td>
                  <button className="button" onClick={() => updateStock(item.id, -1)}>-</button>
                  <button className="button" onClick={() => updateStock(item.id, 1)}>+</button>
                </td>
              </tr>
            ))}
